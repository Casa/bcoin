...
class MTX extends TX {
  ...
  signInput(index, coin, ring, type) {
    return this.addSig(index, coin, null, ring, type);
  }
   ...
   * @param {Number} index - Index of input being signed.
   * @param {Coin|Output} coin
   * @param {Buffer?} sig - not required if ring has a private key
   * @param {KeyRing} ring
   * @param {SighashType?} type - not required if `sig` is present
   * @returns {Boolean} Whether the input was able to be signed.
   */

  addSig(index, coin, preSig, ring, type) {
    const input = this.inputs[index];
